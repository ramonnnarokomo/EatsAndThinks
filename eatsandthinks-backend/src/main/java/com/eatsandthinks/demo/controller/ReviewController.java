...
public class ReviewController {
    ...
    public ResponseEntity<?> createReview(
        ...
        try {
            ...
            if (!user.getCanReview()) {
                return ResponseEntity.status(403).body(Map.of("message", "No tienes permisos para publicar reseñas"));
            }
            if ("GUEST".equals(user.getRole())) {
                return ResponseEntity.status(403).body(Map.of("message", "Los invitados no pueden publicar reseñas. Por favor, regístrate."));
            }
